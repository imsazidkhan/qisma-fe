 ...
 *
 * Resolution order:
 *   1. `EXPO_PUBLIC_API_BASE_URL` env var (the only thing we trust in production).
 *      If you pass an **origin only** (e.g. `http://192.168.1.5:3000`), `/v1` is
 *      appended automatically — auth-service routes all APIs under `/v1`.
 *   2. In dev, fall back to the host the Metro bundler is reachable on:
 ...
 *
 * In production builds without `EXPO_PUBLIC_API_BASE_URL` we throw — silently
 * falling back to localhost would leak dev traffic into a release.
 */
export function resolveApiBaseUrl(): string {
  ...
  if (!__DEV__) {
    throw new Error(
      'EXPO_PUBLIC_API_BASE_URL is not set. Set it in your build config before shipping.',
    );
  }
